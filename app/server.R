...
sql2 <- readLines("C:\\Users\\Repro\\Documents\\R\\ADM\\SHINY\\app\\SQL\\result_fat.sql")
result_fat <- paste(sql2, collapse = " ")

result_fat <- dbGetQuery(con2,result_fat)


result_fat <-
  result_fat %>% group_by(SETOR) %>% summarize(VRVENDA=sum(VRVENDA))


## ALCANCE

result_fat2 <- left_join(result_fat,metas2,by="SETOR") %>%
  mutate(ALCANCE=(VRVENDA/META)*100) %>% 
  mutate(SETOR=substr(SETOR,1, 8)) %>% 
  mutate(ESPERADO=((META/num_days)*num_days_2/META)*100)
...
shinyServer(function(input, output) {
    ...
    output$sales_fat <- renderPlot({
      ...
      ggplot() +
        ...
        geom_text(data = result_fat2, aes(x = fct_rev(SETOR), y = ALCANCE, label = percent(round(ALCANCE,1)/100)), 
                  position = position_dodge(width = .7), vjust = 0.5, hjust = -0.1,size=4.5) +
        scale_fill_manual(values = c(bullet_colors, ESPERADO = "#f4d27a", ALCANCE = "#458b00"), 
                          breaks = c("ESPERADO", "ALCANCE")) +
        coord_flip(expand = FALSE) + theme(axis.title.y=element_blank(),
                                           axis.text.y = element_text(size = 13)) +
        theme(panel.background = element_rect(fill = "transparent"),
